module chorus_WAVE
  ...
  public :: init_chorus, update_chorus, update_chorus_adv, interp_ampl, interp_intf, intgl_source
...
contains
  ...
  subroutine update_chorus(chorus, eon)
  ...
  subroutine update_chorus_adv(chorus, eon)
    ! solve the wave amplitude complex equations
    implicit none
    type(chorus_mode), intent(inout) :: chorus(-NZ:NZ+1)
    type(energetic_eon), intent(inout) :: eon(-NZ:NZ,NJ)
    ...
    call implicit_upwind(chorus,eon,dT)
    !call upwind_1st(chorus, dT)
    return
  end subroutine update_chorus_adv


  subroutine implicit_upwind(chorus, eon,dtstp)
    implicit none
    real(fp), intent(in) :: dtstp
    type(chorus_mode), intent(inout) :: chorus(-NZ:NZ+1)
    type(energetic_eon), intent(inout) :: eon(-NZ:NZ)
    real(fp), parameter :: implicit = 1.0_fp
    real(fp) :: dZ, vg(-NZ:NZ+1)
    ...
    do k = -NZ+1, NZ+1
       dZ = chorus(k)%zpos-chorus(k-1)%zpos
       chorus(k)%ampl(1) = (0.5_fp*mathj*vg(k)/chorus(k)%kmode*chorus(k)%Sr    &
                         + implicit*vg(k)/dZ*chorus(k-1)%ampl(1)+(1.0_fp/dtstp    &
                         - (1.0_fp-implicit)*vg(k)/dZ)*ampl(k)                 &
                         + (1.0_fp-implicit)*vg(k)/dZ*ampl(k-1))               &
                         / (1.0_fp/dtstp+implicit*vg(k)/dZ)
    end do
  ...
  subroutine upwind_1st(chorus, dtstp)
    ...
    do k = -NZ+1, NZ
        dz = chorus(k-1)%zpos - chorus(k)%zpos
        ...
        chorus(k)%ampl(1) = ampl_pre(k) - vg*(ampl_pre(k-1) - ampl_pre(k))/dz*dtstp + Sr*dtstp
    end do
        chorus(-NZ)%ampl(1)= lft_bnd*(1.0_fp+noise*norm_rand(0.0_fp,1.0_fp))
  ...
  subroutine intgl_source(eon)
    ...
    return
  end subroutine intgl_source

  subroutine interp_ampl(eon,chorus)
   implicit none
